...
addDepartment = () => {
  inquirer
    ...
    .then((answers) => {
      ...
      db.promise()
        .query(sql, answers.addDept)
        .then(() => {
          console.log(`Added ${answers.addDept} to deparment`);
          showDepartments();
        })
        .catch((error) => console.log("error-> ", error));
    });
    ...
    .then((answer) => {
      ...
      db.promise()
        ...
        .then((data) => {
          ...
          const dept = data[0].map(({ department_name, id }) => ({
  ...
  inquirer
    ...
    .then((answer) => {
      ...
      db.promise()
        .query(roleSql)
        .then((data) => {
          const roles = data[0].map(({ id, title }) => ({
            name: title,
            value: id,
          }));
          inquirer
...
updateEmployee = () => {
  ...
  db.promise()
    .query(sql)
    .then((data) => {
      const employees = data[0].map(({ first_name, last_name, id }) => ({
        name: first_name + " " + last_name,
        value: id,
      }));
      inquirer
...
updateManager = () => {
  ...
  db.promise()
    ...
    .then((data) => {
      ...
      // show managers data
      const managers = data[0].map(({ first_name, last_name, id }) => ({
        name: first_name + " " + last_name,
        value: id,
      }));
      inquirer
...
employeeDepartment = () => {
  ...
  db.promise()
    .query(sql)
    .then((data) => {
      const department = data[0].map(({ id, department_name }) => ({
        name: department_name,
        value: id,
      }));
      inquirer
...
WHERE department_id=?`;
          db.promise()
            .query(employeeSql, departmentId)
            .then(([rows]) => {
              console.table(rows);
              promptUser();
            });
...
employeeManager = () => {
  ...
  db.promise()
    ...
    .then((data) => {
      // console.log(data);
      const manager = data[0].map(({ id, manager }) => ({
        name: manager,
        value: id,
      }));
...
deleteDepartment = () => {
  ...
  db.promise()
    .query(deptSql)
    .then((data) => {
      const dept = data[0].map(({ id, department_name }) => ({
        name: department_name,
        value: id,
      }));
      inquirer
        ...
        .then((deleChoice) => {
          ...
          db.promise()
            .query(sql, dept)
            .then(() => {
              console.log("Suceessful deleted");
              showDepartments();
            });
        });
    });
...
deleteEmployee = () => {
  ...
  db.promise()
    .query(empSql)
    .then((data) => {
      const employee = data[0].map(({ id, first_name, last_name }) => ({
        name: first_name + " " + last_name,
        value: id,
      }));
      inquirer
